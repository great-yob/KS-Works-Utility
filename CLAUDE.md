...
- `POST /api/hwp-image/scan` / `POST /api/hwp-image/convert` (`api-modules/hwpImage.ts`) — spawn the HWP worker; convert streams NDJSON and finishes with a `complete` event carrying `outputPath`/`outputDir`. Output is written beside the original as `변환_<name>.hwp(x)`. Images are converted **in page order** (`sort_by_page`) so the log reads like the document — `BinData` order is the order 한글 stored them (the author's insertion order), which has nothing to do with the final layout; conversion itself is order-independent. `progress`/`size` lines also carry `page` (1-based, the page the image sits on in the original document) and `pages` when one image is placed in several spots — see "Image → page mapping".
...
- **`hwp_pageapi.py`** (정확한 쪽 번호): asks 한글 itself over COM automation. The layout-derived numbers in `hwp_pagemap.py` cannot follow a table that splits across pages, so when 한글 is installed the worker replaces them with exact values — see "Asking 한글 for exact pages" below. Failure at any step (no 한글, dialog, timeout) silently leaves the estimates in place.
...
### Asking 한글 for exact pages (`hwp_pageapi.py`)

The estimator above is only as good as what the file records, and a table that splits across pages records nothing. 한글 itself knows — its status bar shows `135/142쪽` because it just laid the document out — so when 한글 is installed the worker asks it and overwrites the estimates (`apply_exact_pages`). Everything is best-effort: any failure leaves the estimates untouched.

- **The call**: `hwp.SetPos(0, para, pos)` then `hwp.KeyIndicator()[3]`, which returns the **printed** page (새 번호로 시작 already applied — the same basis as the chips). `hwp_pagemap` therefore records, next to every page it computes, the anchor `(top-level paragraph index, char position)` that produced it; those are exactly the coordinates 한글 wants, and they match `ctrl.GetAnchorPos(0)` verbatim (verified).
- **Speed is all about how you move the caret.** `SetPosBySet(ctrl.GetAnchorPos(0))` is ~4 ms; `SetPos(0, para, pos)` costs ~100 ms on a 144 MB document (269 anchors = 29 s). So the resolver walks the `gso` control chain, reads each anchor with a single `GetPos()`, and only falls back to `SetPos` for anchors no control owns (표 셀 배경 등). Do **not** walk `tbl` controls for this — moving the caret into the big tables of a 교재 pushed one document past 45 s. Measured end state: 6 documents, 8–269 anchors, **1.5–2.5 s each**, 171 MB included.
- **Orphans**: a timed-out attempt leaves a hidden 한글 holding the document, and the *next* run then crawls (that, not the algorithm, was behind every slow measurement during development). The resolver snapshots `Hwp.exe` PIDs around `Dispatch`, remembers the one it created, and terminates only that PID when it abandons the thread.
- **Isolation**: a separate `Dispatch` gets its own 한글 instance — opening a document and quitting does not touch a session the user has open (verified: the other session kept its document and page count).
- **The security dialog** is the one thing that hangs `Open()`. `resources/hwp_automation/FilePathCheckerModule.dll` (한컴's official module, shipped by `extraResources`) is copied to `%LOCALAPPDATA%\KS-Works-Utility\` and registered under `HKCU\Software\HNC\HwpAutomation\Modules` on first use; an existing registration by another tool is left alone.
- Accuracy after this: the 회의자료 that started this work reports 5·31·135–140, matching 한글's 그림 정보 exactly, and the 연구보고서's chapter band lands on 139 instead of 132. Documents the estimator already got right are unchanged (0 of 269 and 0 of 229 images moved in two 교재).

### Packaging the worker (`build_hwp_worker.bat`)

The build must also list `hwp_pageapi` alongside `hwp_pagemap`. PyInstaller must be given hidden imports the static analyzer misses because they're imported lazily inside functions — `pythoncom`/`pywintypes` and the sibling `worker` module, and critically **`win32timezone`**: enumerating the OLE `BinData` storage (`enum.Next()`) converts STATSTG timestamps and lazily imports `win32timezone`; if it's missing, the enumeration throws, gets swallowed, and **scan/convert silently return 0 images** in the frozen exe while working fine under dev Python.
